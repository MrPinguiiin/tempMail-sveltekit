...
export const GET: RequestHandler = async ({ url, platform }) => {
	...
	const stream = new ReadableStream({
		async start(controller) {
			...
			const fetchEmails = async () => {
				try {
					...
					if (dev) {
						console.error('[Redis SSE] Error fetching emails:', error);
					}
					return [];
				}
			};
			...
			const setupRedis = async () => {
				try {
					...
					redisClient.on('error', (err: any) => {
						console.error('[Redis SSE] Redis Client Error:', err);
						isConnected = false;
					});

					redisClient.on('connect', () => {
						console.log('[Redis SSE] Redis connected');
						isConnected = true;
					});

					redisClient.on('disconnect', () => {
						console.log('[Redis SSE] Redis disconnected');
						isConnected = false;
					});

					await redisClient.connect();

					// Subscribe to email updates for this specific email
					await redisClient.subscribe(`email:${email}`, (message: string) => {
						console.log('[Redis SSE] Received message:', message);
						
						// Fetch fresh emails when we get a notification
					...
					console.log(`[Redis SSE] Subscribed to email:${email}`);
					
				} catch (error) {
					console.error('[Redis SSE] Failed to setup Redis:', error);
					isConnected = false;
				}
			};
		...
		cancel() {
			if (dev) {
				console.log('[Redis SSE] Client disconnected');
			}
		}
	});
